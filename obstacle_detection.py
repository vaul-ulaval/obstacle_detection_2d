...
def obstacle_detection(map: Map, pose: Pose, scan: Scan):
    ...
    angles = (
        scan.metadata.angle_min + np.arange(ranges.size) * scan.metadata.angle_increment
    )
...
def is_wall(coord, map: Map, thickened_map):
    row, col = world_to_map_indices(map, coord[0], coord[1])

    return thickened_map[row, col] == 100


def get_thickened_map(map: Map, wall_thickness=10):
    walls = map.grid == 100
...
def remove_walls(obstacles, map: Map):
    thickened_map = get_thickened_map(map)
    ...
    for obs in obstacles:
        ...
        for coords in obs:
            ...
            if is_wall((x, y), map, thickened_map):
                obs.clear()
                break
